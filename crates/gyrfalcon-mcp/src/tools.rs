...
impl Profile {
    ...
    fn allows(self, name: &str) -> bool {
        ...
        //
        // `check_bsl` — тоже в обоих, и это не щедрость: он ничего не читает
        // из индекса и не меняет, а разбирает поданный текст. Роль, которая
        // пишет код, без него пишет вслепую, а какая именно роль пишет —
        // решается заданием, не профилем.
        const SCOUT: &[&str] = &[
            "list_projects",
            "find",
            "object",
...
pub const TOOLS: &[Tool] = &[
    ...
    Tool {
        name: "check_bsl",
        description: "Проверить синтаксис модуля BSL ДО записи в файл — структура кода: \
             незакрытые блоки, скобки, Если без Тогда. Отдаёт строку, колонку и сам фрагмент. \
             ВАЖНО, чтобы зелёный ответ не ввёл в заблуждение: имена методов и реквизитов, \
             число аргументов НЕ проверяются — это индекс, а не разбор текста. \
             Текст ЗАПРОСА проверяется не здесь, а в канале данных (query_check).",
        schema: crate::check_bsl::schema,
    },
    Tool {
        name: "check_names",
        description: "Сверить ИМЕНА в модуле BSL с индексом конфигурации — то, чего разбор              текста не видит: выдуманный метод, не то число аргументов, чужой реквизит.              Проверяются только вызовы «Модуль.Метод», где Модуль — объект ЭТОЙ конфигурации;              встроенные функции платформы и обращения через переменные пропускаются, потому              что индекс о них не знает. Звать вместе с check_bsl перед записью правки:              тот проверяет структуру, этот — имена.",
        schema: crate::check_names::schema,
    },
    Tool {
        name: "sql",
...
pub fn call(
    ...
    match name {
        ...
        // Единственный инструмент, которому индекс не нужен вовсе: он разбирает
        // поданный текст, а не спрашивает конфигурацию. Отсюда и `conn` мимо.
        "check_bsl" => crate::check_bsl::check_bsl(args),
        "check_names" => crate::check_names::check_names(conn, args),
        "sql" => {
